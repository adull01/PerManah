        ...
        <table class="modern-table">
            ...
            <tbody>
                ...
                <tr class="{{ $borrowing->status == 'pending' && $borrowing->notified_at ? 'highlight' : '' }} {{ $borrowing->overdue_status == 'late' ? 'table-danger' : '' }}">
                    ...
                    <td>
                        @if($borrowing->overdue_status == 'late')
                    ...
                    <td>
                        ...
                        @if($lateFee > 0)
                            @if($borrowing->is_fee_paid ?? false)
                                <span class="badge bg-success" title="Lunas">Rp{{ number_format($lateFee, 0, ',', '.') }} (Lunas)</span>
                            @else
                                <span class="badge bg-danger" title="Belum Lunas">Rp{{ number_format($lateFee, 0, ',', '.') }}</span>
                            @endif
                            <div class="mt-1">
                                        ...
                                        <form action="{{ route('petugas.borrowings.mark-lost', $borrowing->id) }}" method="POST">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="number" name="replacement_fee" class="form-control" placeholder="Biarkan kosong untuk menggunakan harga buku ({{ number_format($borrowing->book->price ?? 0,0,',','.') }})">
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                                        ...
                                        <form action="{{ route('petugas.payments.store') }}" method="POST">
                                            ...
                                            <div class="modal-body">
                                                <div class="mb-3">
                                                    ...
                                                    <input type="number" name="amount" class="form-control" value="{{ ($borrowing->late_fee ?? 0) + ($borrowing->replacement_fee ?? 0) }}" required>
                                                </div>
                                                <div class="mb-3">
