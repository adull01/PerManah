...
<div class="stats-grid">
    ...
    <div class="stat-card warning">
        ...
        <div class="stat-footer">
            <a href="{{ route('petugas.borrowings') }}">
...
<div class="notification-card">
    <div class="notification-header">
        ...
        <span class="notification-badge">{{ $newNotifications->count() }}</span>
    </div>
    <div class="table-responsive">
        <table class="modern-table">
            ...
            <tbody>
                @foreach($newNotifications as $borrowing)
                <tr class="highlight">
                    <td><strong>#{{ $borrowing->book->id }}</strong></td>
                    <td>
                        <strong>{{ $borrowing->user->name }}</strong><br>
                        <small class="text-muted">ID: #{{ $borrowing->user->id }}</small>
                    </td>
                    <td>{{ $borrowing->book->title }}</td>
                    <td>
                        @if($borrowing->notified_at)
                            <i class="fas fa-clock"></i> {{ $borrowing->notified_at->diffForHumans() }}<br>
                            <small class="text-muted">{{ $borrowing->notified_at->format('d/m/Y H:i') }}</small>
                        @else
                            <i class="fas fa-clock"></i> {{ $borrowing->created_at->diffForHumans() }}<br>
                            <small class="text-muted">{{ $borrowing->created_at->format('d/m/Y H:i') }}</small>
                        @endif
                    </td>
                    <td>
                        @if($borrowing->book->available > 0)
                            <span class="badge bg-success">Tersedia: {{ $borrowing->book->available }}</span>
                        @else
                            <span class="badge bg-danger">Stok Habis</span>
                        @endif
                    </td>
                    <td>
                        <a href="{{ route('petugas.borrowings') }}" class="btn-action-dashboard btn-process">
