...
<body>
    ...
    <div class="stats">
        ...
        <div class="stat">Total Denda: {{ number_format($totalDenda, 0, ',', '.') }}</div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $borrowing->is_lost ? 'Hilang' : ucfirst($borrowing->status) }}</td>
                <td>{{ number_format(($borrowing->is_lost ?? false) ? ($borrowing->replacement_fee ?? 0) : (($borrowing->late_fee ?? 0) + ($borrowing->replacement_fee ?? 0)), 0, ',', '.') }}</td>
            </tr>
            @empty
            <tr>
